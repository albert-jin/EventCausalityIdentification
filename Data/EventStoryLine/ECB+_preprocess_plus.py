...
def extract_from_three_factors(causalities, events, tokens, negative_ratio=3, non_cause_tag='NULL'):
    ...
    for causality in causalities_all:
        try:
            ...
            event1_indexs, event2_indexs = (s_event_indexs, e_event_indexs) if int(s_event_indexs[0]) - int(e_event_indexs[0])<0 else (e_event_indexs, s_event_indexs)
            if int(event2_indexs[0]) - int(event1_indexs[-1]) > 100:  # Rule3
                continue
            # 寻找开始事件前倒序第一个的句子截断符号→ 错，修改为寻找第一个事件出现前倒序第一个的句子截断符号

            sent_start_idx = int(event1_indexs[0])
            ...
            if int(event1_indexs[0]) - sent_start_idx > 50:  # Rule2
                sent_start_idx = int(event1_indexs[0]) - 50

            # 寻找结束事件后正序第一个的句子截断符号→ 错，修改为寻找第二个事件出现后倒序第一个的句子截断符号
            sent_end_idx = int(event2_indexs[-1])
            while str(sent_end_idx) in tokens:  # 循环结束，sent_end_idx 为句子结束的下标+1
                if tokens[str(sent_end_idx)] in STOP_TOKENS:
                    sent_end_idx += 1
                    break
                sent_end_idx += 1
            if sent_end_idx - int(event2_indexs[-1]) > 50:  # Rule2
                sent_end_idx = int(event2_indexs[-1]) + 50
